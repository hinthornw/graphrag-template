...
from chatbot.utils import format_memories, query_knowledge_graph
...
"""Example chatbot that connects to the memory processor graph."""
...
async def bot(
    ...
    items = await query_knowledge_graph(
        query=query, user_id=configurable.user_id, store=store, limit=10
    )
    ...
    m = await model.ainvoke(
        ...
        {"configurable": asdict(configurable)},
    )
